...
export async function POST(req: NextRequest) {
  ...
  const sides = buildSides(scores);

  return NextResponse.json(sides);
}

const buildSides = (players: { player_id: string; score: number }[]) => {
  ...
  for (const size of [1, 2, 3]) {
    for (const side of combos(player_ids, size)) {
      const scoreSide = sum(side, score);
  ...
  return sides.sort(() => Math.random() - Math.random()).slice(0, 5);
