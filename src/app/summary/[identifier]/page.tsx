    ...
    <div className="flex flex-col items-center text-center">
      ...
      {summary.user_ranks.length > 0 && summary.lm_ranks.length > 0 && (
        <div className="w-full">
          <h1 className="mb-8">{fairTrades.length} Recommended Trades</h1>
          <ol>
            {fairTrades
              .sort(
                (a, b) =>
                  Math.min(b.userDiff, b.lmDiff) -
                  Math.min(a.userDiff, a.lmDiff)
              )
              .map((ft) => {
                return (
                  <li
                    key={ft.user.join("_") + ft.lm.join("_")}
                    className="w-full flex mb-8 bg-gray-600 filter contrast-200"
                  >
